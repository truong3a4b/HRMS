    ...
    <article className="group relative overflow-hidden rounded-2xl bg-white p-5 shadow-[0_4px_24px_rgba(0,0,0,0.06)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgb(0,0,0,0.12)] border border-slate-100/80">
      <div className="relative z-10 flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <p className="mb-1 text-sm font-semibold text-slate-500 group-hover:text-slate-600 transition-colors">
            {title}
          </p>
        ...
        <div
          className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${tone} transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3 shadow-sm *:h-5 *:w-5`}
        >
          {icon}
        </div>
      </div>
...
function Section({
  ...
  return (
    <section
      className={`rounded-2xl bg-white p-6 shadow-[0_4px_24px_rgb(0,0,0,0.05)] border border-slate-100/80 transition-shadow duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] flex flex-col ${className}`}
    >
      <div className="mb-5">
        ...
        {subtitle && (
          <p className="mt-1.5 ml-[14px] text-sm font-medium text-slate-500">
            {subtitle}
          </p>
        )}
      </div>
...
function MetricRow({
  ...
  return (
    <div className="group flex items-center justify-between gap-4 rounded-xl px-3 py-3 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-b-0">
      ...
      <strong className="text-sm font-bold text-slate-900 bg-slate-100 px-3 py-1 rounded-lg group-hover:bg-white group-hover:shadow-sm transition-all">
        {value}
      </strong>
    </div>
  );
...
export function HomeDashboard() {
  ...
  // Load dashboard data when selectedDate changes
  useEffect(() => {
    let alive = true;
    ...
    async function load() {
      ...
      try {
        ...
        const dayMapByEmployee = new Map<
          string,
          Map<string, AttendanceTimesheetDay>
        >();
        ...
        const payrollOverview = currentPeriod
          ? await settle(
              payrollService.getPeriodOverview(currentPeriod.id),
              null,
            )
          : null;
  ...
  // Tinh toán tổng quan nhân sự dựa trên dữ liệu hiện tại và ngày được chọn
  const employeeOverview = useMemo(() => {
    const selectedMonth = selectedDate.slice(0, 7);
    const working = state.employees.filter(
      (employee) => employee.status === "WORKING",
    ).length;
    const onLeave = state.employees.filter(
      (employee) => employee.status === "ON_LEAVE",
    ).length;
    const resigned = state.employees.filter(
      (employee) => employee.status === "RESIGNED",
    ).length;
  ...
  const recruitment = useMemo(() => {
    ...
    return {
      openJobs: state.recruitmentJobs.filter((job) => job.status === "OPEN")
        .length,
      ...
      interviewsToday: state.applications.reduce(
        (total, application) =>
          ...
          (application.interviewSchedules?.filter(
            (schedule) =>
              toDateKey(new Date(schedule.scheduledAt)) === selectedDate,
          ).length ?? 0),
        0,
      ),
      pendingOffers: state.applications.reduce(
        (total, application) =>
          total +
          (application.offers?.filter((offer) => offer.status === "SENT")
            .length ?? 0),
        0,
      ),
  ...
  const payrollStatus =
    state.payrollOverview?.period.status ??
    state.payrollFallback?.status ??
    "Chưa tạo";
  ...
  const weekStartDateLabel = weekStart.toLocaleDateString("vi-VN", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
  });
  const weekEndDateLabel = new Date(
    `${selectedDate}T00:00:00`,
  ).toLocaleDateString("vi-VN", {
    day: "2-digit",
    month: "2-digit",
    ...
    <main className="flex-1 overflow-y-auto bg-[#f8fafc] px-8 py-8 max-[720px]:px-4">
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-6 max-[1280px]:grid-cols-3 max-[760px]:grid-cols-1">
            <StatCard
          ...
          <div className="grid grid-cols-[1fr_1fr_1fr] gap-6 max-[1280px]:grid-cols-1">
            ...
            <Section title={`Payroll tháng ${selectedMonthLabel}`}>
              <div className="flex flex-col h-full justify-evenly">
                ...
                <MetricRow
                  label="Đã thanh toán"
                  value={formatMoney(paidPayroll)}
                />
                <MetricRow
                  label="Còn lại"
                  value={formatMoney(
                    Math.max(expectedPayroll - paidPayroll, 0),
                  )}
                />
              </div>
            </Section>
          ...
          <div className="grid grid-cols-2 gap-6 max-[1080px]:grid-cols-1">
            ...
            <Section title="Tuyển dụng">
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 max-[520px]:grid-cols-1">
                ...
                <MetricRow
                  label="Ứng viên mới"
                  value={recruitment.newApplications}
                />
                <MetricRow
                  label="Phỏng vấn hôm nay"
                  value={recruitment.interviewsToday}
                />
                <MetricRow
                  label="Offer chờ phản hồi"
                  value={recruitment.pendingOffers}
                />
                <MetricRow
                  label="Apply trong tháng"
          ...
          <div className="grid grid-cols-3 gap-6 max-[1280px]:grid-cols-1">
            ...
            <Section
              title="Đi muộn, về sớm, vắng theo tuần"
