  ...
  return (
    <div ref={ref} className="relative min-w-0 max-w-full">
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className={`flex h-9 w-full min-w-0 max-w-full items-center justify-between gap-2 rounded-lg border px-3 text-sm transition-colors ${
          open
...
export function ScheduleAssignPage() {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      setErrorMessage("Không tải được dữ liệu cấu hình lịch làm việc");
    });
  }, []);

  const validateForm = () => {
    if (!name.trim()) return "Vui lòng nhập tên cấu hình lịch.";
    if (selectedDepartments.length === 0 && selectedPositions.length === 0)
      return "Vui lòng chọn phòng ban hoặc chức vụ áp dụng.";
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSuccessMessage(null);
    try {
      await scheduleService.createSetup({
        name: name.trim(),
        description: description.trim() || undefined,
        applicableDepartments: selectedDepartments,
        applicablePositions: selectedPositions,
        scheduleDetails,
      });
      setSuccessMessage("Đã áp lịch làm việc cho nhân viên phù hợp.");
    } catch (error) {
      setErrorMessage(getErrorMessage(error, "Không thể áp lịch làm việc"));
    } finally {
              ...
              <div className="grid content-start gap-4">
                ...
                <label>
                  <span className={labelClass}>
                    Tên cấu hình lịch <span className="text-[#f04438]">*</span>
                  </span>
                  <input
                    className={fieldClass}
                    value={name}
                    placeholder="VD: Lịch hành chính tháng 06"
                ...
                <label>
                  ...
                  <textarea
                    ...
                    placeholder="Ghi chú thêm về cấu hình lịch này..."
                    value={description}
                ...
                {/* Info note */}
                <div className="flex items-start gap-2 rounded-lg border border-[#e0ecff] bg-[#f0f7ff] px-3 py-2.5 text-xs text-[#3b6ab5]">
                  <span className="mt-0.5 shrink-0">ℹ️</span>
                  <span>
                    Lịch chỉ được áp cho nhân viên thuộc phòng ban hoặc chức vụ
                    đã chọn, và chỉ áp cho ngày tương lai.
                  </span>
                </div>
              </div>
            </section>
