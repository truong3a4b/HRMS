...
class RecruitmentRemote {
  ...
  Future<List<RecruitmentJobDto>> fetchRecruitmentJobs({
    ...
    try {
      ...
      throw AppException(e.response?.data['message'] ?? 'Lỗi tải vị trí tuyển dụng');
    }
  }
  ...
  Future<RecruitmentJobDto> getRecruitmentJobById(String id) async {
    try {
      ...
      throw AppException(e.response?.data['message'] ?? 'Lỗi tải chi tiết vị trí tuyển dụng');
    }
  }
  ...
  Future<bool> closeRecruitmentJob(String id) async {
    try {
      ...
      throw AppException(e.response?.data['message'] ?? 'Lỗi đóng vị trí tuyển dụng');
    }
  }
  ...
  Future<bool> applyJob(Map<String, dynamic> data) async {
    try {
      ...
      throw AppException(e.response?.data['message'] ?? 'Lỗi ứng tuyển vị trí tuyển dụng');
    }
  }
}
